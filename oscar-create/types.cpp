...
Options::TextSearchConfig::TextSearchConfig(const string& str) :
...
Options::TagStoreConfig::TagStoreConfig(const string& str) : create(true) {
	std::vector<std::string> ks = sserialize::split< std::vector<std::string> >(str, ',', '\\');
	std::vector< std::pair<std::string, std::string> > kvs;
	for(std::vector<std::string>::const_iterator it(ks.cbegin()), end(ks.cend()); it != end; ++it) {
		std::vector<std::string> tmp = sserialize::split< std::vector<std::string> >(*it, '=', '\\');
...
Options::KVStoreConfig::KVStoreConfig(const string& str) :
maxNodeHashTableSize(std::numeric_limits<uint32_t>::max()),
saveEverything(false),
saveEveryTag(false),
minNodeId(0),
...
Options::ReturnValues Options::fromCmdLineArgs(int argc, char** argv) {
	...
	if (m_outFileName.size() == 0) {
		...
		cout << endl << "Need out filename" << endl;
		return RV_FAILED;
	}

	if (inFileNames.size() == 0) {
		cout << endl << "Need in file name" << endl;
	}
...
OsmGeneralizedTrieCreatorConfig Options::toTrieConfig(const TextSearchConfig & cfg) {
	OsmGeneralizedTrieCreatorConfig config;
...
string Options::getOutFileDir() const {
...
string Options::getOutFileName(liboscar::FileConfig fc) const {
...
std::ostream& operator<<(std::ostream& out, const oscar_create::Options & opts) {
