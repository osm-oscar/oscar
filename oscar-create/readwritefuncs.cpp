...
void handleKVCreation(oscar_create::Config & opts, State & state) {
	...
	if (opts.inFileNames.size()) {
		...
		cc.rc.polyStoreMaxTriangPerCell = opts.kvStoreConfig->cellRefineCfg.maxTriangPerCell;
		cc.rc.triangMaxCentroidDist = opts.kvStoreConfig->triangRefineCfg.maxCentroidDistance;
